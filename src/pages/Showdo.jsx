...
const ShowDo = () => {
  ...
  const fetchDos = async () => {
    try {
      ...
      const response = await axios.get(`${apiBaseUrl}/do-data/`, {
        headers: {
          "api-key": apiKey,
      ...
      if (response.status === 200) {
        ...
        toast.error("Failed to fetch DOs");
      }
    } catch (error) {
      toast.error("Error fetching DOs");
      setIsLoading(false);
    }
  };
  ...
  const performAction = async () => {
    try {
      ...
      if (currentAction === "delete") {
        await axios.delete(`${apiBaseUrl}/delete-do-data/${selectedId}`, {
          headers: {
            "api-key": apiKey,
            Authorization: `Bearer ${token}`,
          },
        ...
        fetchDos(); // Refresh the list
      } else if (currentAction === "update") {
        navigate(`/update-do/${selectedId}`);
      }
    } catch (error) {
                ...
                <table className="min-w-full text-left table-auto border-collapse border border-gray-300">
                  <thead>
                    <tr className="bg-indigo-600 text-white">
                      ...
                      <th className="px-4 py-3 font-semibold">Total Bardana</th>
                      ...
                      dos.map((doItem) => (
                        ...
                        >
                          ...
                          <td className="px-4 py-3 text-gray-600 flex space-x-2">
                            {permissions.update && (
                              <button
                                onClick={() => handleUpdate(doItem.do_id)}
                            ...
                            {permissions.delete && (
                              <button
                                onClick={() => handleDelete(doItem.do_id)}
