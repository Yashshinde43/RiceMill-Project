...
const ShowOtherJawak = () => {
  const [otherJawak, setOtherJawak] = useState([]);
  ...
  // Fetch Other Jawak data
  const fetchOtherJawak = async () => {
    try {
      const token = localStorage.getItem("token");
      const response = await axios.get(`${apiBaseUrl}/other-jawak-data`, {
        headers: {
          "api-key": apiKey,
      ...
      if (response.status === 200) {
        ...
        setOtherJawak(response.data);
        setIsLoading(false);
      } else {
        toast.error("Failed to fetch Other Jawak");
      }
    } catch (error) {
      toast.error("Error fetching Other Jawak");
      setIsLoading(false);
    }
  };
  ...
  const performAction = async () => {
    try {
      ...
      if (currentAction === "delete") {
        await axios.delete(`${apiBaseUrl}/delete-other-jawak/${selectedId}`, {
          headers: {
            "api-key": apiKey,
            Authorization: `Bearer ${token}`,
          },
        });
        toast.success("Other Jawak deleted successfully");
        fetchOtherJawak(); // Refresh the list
      } else if (currentAction === "update") {
        navigate(`/update-other-jawak/${selectedId}`);
      }
    } catch (error) {
      ...
      <div className="min-h-full flex flex-col justify-center py-12 sm:px-6 lg:px-8">
        <div className="sm:mx-auto sm:w-full sm:max-w-md">
          <h2 className="mt-6 text-center text-3xl font-extrabold text-gray-900">
            Other Jawak Directory
          </h2>
          <p className="mt-2 text-center text-sm text-gray-600">
            A list of all the Other Jawak records in the system.
          </p>
        </div>
          ...
          <div className="bg-white py-8 px-6 shadow-md rounded-lg sm:px-10">
            {isLoading ? (
              <div className="flex justify-center items-center">
                ...
                <p className="ml-4 text-indigo-600">Loading Other Jawak...</p>
              </div>
            ) : (
      ...
      <ConfirmationModal
        ...
        message={
          currentAction === "delete"
            ? "Are you sure you want to delete this Other Jawak?"
            : "Are you sure you want to update this Other Jawak?"
        }
      />
    </SideBar>
  );
