...
const ShowKochia = () => {
  const [kochiaList, setKochiaList] = useState([]);
  ...
  const fetchKochia = async () => {
    try {
      ...
      const response = await axios.get(`${apiBaseUrl}/kochia-data/`, {
        headers: {
          "api-key": apiKey,
      ...
      if (response.status === 200) {
        setKochiaList(response.data);
        setIsLoading(false);
      } else {
        toast.error("Failed to fetch Kochia list");
      }
    } catch (error) {
      toast.error("Error fetching Kochia list");
      setIsLoading(false);
    }
  };
  ...
  const performAction = async () => {
    try {
      ...
      if (currentAction === "delete") {
        await axios.delete(`${apiBaseUrl}/delete-kochia/${selectedId}`, {
          headers: {
            "api-key": apiKey,
            Authorization: `Bearer ${token}`,
          },
        });
        toast.success("Kochia deleted successfully");
        fetchKochia(); // Refresh the list
      } else if (currentAction === "update") {
        navigate(`/update-kochia/${selectedId}`);
      }
    } catch (error) {
          ...
          <div className="bg-white py-8 px-6 shadow-md rounded-lg sm:px-10">
            {isLoading ? (
              <div className="flex justify-center items-center">
                ...
                <p className="ml-4 text-indigo-600">Loading Kochia...</p>
              </div>
            ) : (
                      ...
                      kochiaList.map((kochia) => (
                        ...
                        >
                          ...
                          <td className="px-4 py-3 text-gray-600 flex space-x-2">
                            {permissions.update && (
                              <button
                                onClick={() => handleUpdate(kochia.kochia_id)}
                            ...
                            {permissions.delete && (
                              <button
                                onClick={() => handleDelete(kochia.kochia_id)}
      ...
      <ConfirmationModal
        ...
        message={
          currentAction === "delete"
            ? "Are you sure you want to delete this Kochia?"
            : "Are you sure you want to update this Kochia?"
        }
      />
    </SideBar>
  );
