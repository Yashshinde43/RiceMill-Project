...
const ShowBrokers = () => {
  ...
  const fetchBrokers = async () => {
    try {
      ...
      const response = await axios.get(`${apiBaseUrl}/broker-data/`, {
        headers: {
          "api-key": apiKey,
      ...
      toast.error("Error fetching brokers");
      setIsLoading(false);
    }
  };
  ...
  const performAction = async () => {
    try {
      ...
      if (currentAction === "delete") {
        await axios.delete(`${apiBaseUrl}/delete-broker-data/${selectedId}`, {
          headers: {
            "api-key": apiKey,
            Authorization: `Bearer ${token}`,
          },
        ...
        navigate(`/update-broker/${selectedId}`);
      }
    } catch (error) {
          ...
          <div className="bg-white py-8 px-6 shadow-md rounded-lg sm:px-10">
            {isLoading ? (
              <div className="flex justify-center items-center">
                ...
                <p className="ml-4 text-indigo-600">Loading brokers...</p>
              </div>
            ) : (
                      ...
                      brokers.map((broker) => (
                        ...
                        >
                          ...
                          <td className="px-4 py-3 text-gray-600 flex space-x-2">
                            {permissions.update && (
                              <button
                                onClick={() => handleUpdate(broker.broker_id)}
                            ...
                            {permissions.delete && (
                              <button
                                onClick={() => handleDelete(broker.broker_id)}
      ...
      <ConfirmationModal
        ...
        message={
          currentAction === "delete"
            ...
            : "Are you sure you want to update this broker?"
        }
      />
    </SideBar>
  );
