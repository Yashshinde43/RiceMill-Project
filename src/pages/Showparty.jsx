...
const ShowParty = () => {
  ...
  const fetchParties = async () => {
    try {
      ...
      const response = await axios.get(`${apiBaseUrl}/party-data/`, {
        headers: {
          "api-key": apiKey,
  ...
  const performAction = async () => {
    try {
      ...
      if (currentAction === "delete") {
        await axios.delete(`${apiBaseUrl}/delete-party/${selectedId}`, {
          headers: {
            "api-key": apiKey,
            Authorization: `Bearer ${token}`,
          },
        ...
        navigate(`/update-party/${selectedId}`);
      }
    } catch (error) {
          ...
          <div className="bg-white py-8 px-6 shadow-md rounded-lg sm:px-10">
            {isLoading ? (
              <div className="flex justify-center items-center">
                ...
                <p className="ml-4 text-indigo-600">Loading parties...</p>
              </div>
            ) : (
                      ...
                      parties.map((party) => (
                        ...
                        >
                          ...
                          <td className="px-4 py-3 text-gray-600 flex space-x-2">
                            {permissions.update && (
                              <button
                                onClick={() => handleUpdate(party.party_id)}
