...
const UpdateBroker = () => {
  ...
  const fetchBroker = async () => {
    try {
      ...
      const response = await axios.get(`${apiBaseUrl}/broker-data-by-id/${id}`, {
        headers: {
          "api-key": apiKey,
  ...
  const handleSubmit = async () => {
    try {
      ...
      const response = await axios.put(`${apiBaseUrl}/update-broker-data/${id}`, broker, {
        headers: {
          "api-key": apiKey,
          Authorization: `Bearer ${token}`,
        },
  ...
  // Fetch broker data on component mount
  useEffect(() => {
    const token = localStorage.getItem("token");
    if (!token) {
      navigate("/login");
            ...
            {isLoading ? (
              ...
              <form onSubmit={handleFormSubmit}>
                <div className="space-y-4">
                  ...
                  <div>
                    <label htmlFor="broker_phone_number" className="block text-sm font-medium text-gray-700">
                      Broker Phone Number
                    </label>
                    <input
                      type="text"
                      ...
                      value={broker.broker_phone_number}
                      onChange={handleChange}
                      required
                      className="mt-1 block w-full border border-gray-300 rounded-md p-2"
                    />
