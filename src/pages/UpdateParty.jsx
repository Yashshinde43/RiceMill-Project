...
const UpdateParty = () => {
  ...
  const fetchParty = async () => {
    try {
      ...
      const response = await axios.get(`${apiBaseUrl}/party-data-by-id/${id}`, {
        headers: {
          "api-key": apiKey,
      ...
      if (response.status === 200) {
        ...
        toast.error("Failed to fetch party data");
        setIsLoading(false);
      }
    } catch (error) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const response = await axios.put(`${apiBaseUrl}/update-party/${id}`, party, {
        headers: {
          "api-key": apiKey,
          Authorization: `Bearer ${token}`,
        },
            ...
            {isLoading ? (
              ...
              <form onSubmit={handleFormSubmit}>
                <div className="space-y-4">
                  ...
                  <div>
                    <label htmlFor="party_phone_number" className="block text-sm font-medium text-gray-700">
                      Party Phone Number
                    </label>
                    <input
                      type="text"
                      ...
                      value={party.party_phone_number}
                      onChange={handleChange}
                      required
                      className="mt-1 block w-full border border-gray-300 rounded-md p-2"
                    />
